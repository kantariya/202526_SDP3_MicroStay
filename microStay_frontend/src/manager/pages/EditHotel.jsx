...
const EditHotel = () => {
    ...
    const [formData, setFormData] = useState({
        description: '',
        contact: { phone: '', email: '' },
        facilities: [], // simple array of strings
    ...
    useEffect(() => {
        ...
        const fetchHotel = async () => {
            try {
                // Strictly use allowed API: GET /manager/hotels
                const res = await api.get('/manager/hotels');
                // Client-side finding of the hotel
                const hotel = res.data.find(h => h.id.toString() === id.toString());
                ...
                setFormData({
                    ...
                    policies: hotel.policies || { checkInTime: '', checkOutTime: '', cancellationPolicy: '' }
                });
            } catch (error) {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setFormData({ ...formData, policies: { ...formData.policies, checkInTime: e.target.value } })}
                                className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-slate-200"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setFormData({ ...formData, policies: { ...formData.policies, checkOutTime: e.target.value } })}
                                className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-slate-200"
                            />
                        </div>
                        <div className="md:col-span-3">
                            ...
                            <textarea
                                ...
                                onChange={(e) => setFormData({ ...formData, policies: { ...formData.policies, cancellationPolicy: e.target.value } })}
